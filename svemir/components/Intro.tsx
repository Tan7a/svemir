...
export default function Intro() {
  ...
  useEffect(() => {
    ...
    // hydration, not during render - hence the deferred setState here.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    if (!seen) setPhase("title");
  ...
  // with oscillators so it needs no asset. Only ever triggered by the Enter
  // click (a user gesture), so autoplay policies are satisfied.
  ...
  // Stop any running audio if the overlay leaves early.
  useEffect(() => {
    if (phase !== "idle") return;
    if (audioRef.current) {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex max-w-lg flex-col items-center gap-6">
        ...
        {!playing ? (
          <>
            <p className="text-sm text-neutral-500">a little universe</p>
            <div className="mt-2 flex flex-col items-center gap-3">
              <button
                type="button"
                onClick={enter}
                className="rounded-full border border-neutral-600 px-7 py-2 text-sm text-neutral-100 transition-colors hover:bg-neutral-900"
              >
                Enter →
              </button>
              <button
          ...
          >
            <p className="text-[15px] leading-relaxed text-neutral-300">
              ...
              Croatian, Bosnian and Montenegrin — from the Slavic roots{" "}
              <span className="italic">sve-</span> (&ldquo;all&rdquo;) +{" "}
              <span className="italic">mir</span> (&ldquo;peace / world&rdquo;).
            </p>
